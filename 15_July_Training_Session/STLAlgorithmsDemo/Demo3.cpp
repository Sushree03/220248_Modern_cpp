...
int main(){
    ...
    bool result1 = std::none_of(              //check if none_of my values satisfies the condition
        stackemployees.begin(),
        stackemployees.end(),
        [](const Employee& e) {return e.age() > 25; }
    );

    bool result2 = std::none_of(
        rawpointeremployees.begin(),
        rawpointeremployees.end(),
        [](const Employee* e) {return e->age() > 25; }
    );

    bool result3 = std::none_of(                //result3 variable will be equal to result of all_of
