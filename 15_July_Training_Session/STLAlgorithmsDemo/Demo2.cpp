...
int main(){
    ...
    bool result1 = std::any_of(   //check if any of the condition satisfies
        stackemployees.begin(),
        stackemployees.end(),
        [](const Employee& e) {return e.age() > 25; }
    );

    bool result2 = std::any_of(
        rawpointeremployees.begin(),
        rawpointeremployees.end(),
        [](const Employee* e) {return e->age() > 25; }
    );

    bool result3 = std::any_of(                //result3 variable will be equal to result of all_of
